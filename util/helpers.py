...
from util.url2text import Url2Text
#from util.text2sound import Test2Sound
...
def gen_sound_from_text(text, outputfile):

    ts = Test2Sound(text=text, language='en', outfile=outputfile)

    return ts


def gen_summary_from_text(text):

    ts = Summarize(text)
    result, _ = ts.summarize_text()

    return result

def gen_summary_from_sentences(sentences):

    list_summary = []
    for sentence in sentences:
        s = gen_summary_from_text(sentence)
